...
Train reports aggregate train state, gate state, fault and per-track UP/DOWN state; flashing phase and train STOP remain unreported. Its blocking callbacks, shared simulator concurrency, unconditional ACK and crossing-to-intersection routing need owner review before an integrated real-time claim. The current remote `p#-fault` path re-locks an already-held Train mutex through a callback, so Central blocks that request with an explicit unsent outcome. Console fault injection and a `noexit P# up` train timeout provide alternatives while the Train owner fixes the deadlock. Central-only changes cannot repair the underlying peer code.

The merged Local now implements phases, traffic/time-of-day simulation, pedestrian/railway paths, extended status/fault envelopes, typed per-intersection heartbeat health, mode/coordination receipts, runtime intersection/service configuration, railway-to-intersection filtering and send mutexes. Remaining source-review gaps are the Local `SIM1` counterpart, real deployment of separate I1-I6 Local processes, Train delivery to every affected Local service, injected actuator-fault evidence and target execution of the Local harness. The exact findings are in [INTEGRATION.md](INTEGRATION.md). Continued safe Local operation, temporary expiry with Central offline and simulation-control behavior must be demonstrated using the actual Local process, not inferred from a Central fixture.
